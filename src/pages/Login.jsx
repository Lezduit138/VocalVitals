...
export default function Login() {
  ...
  const handleGuestLogin = async () => {
    try {
      ...
      setErrorText('Failed to sign in as guest.');
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-background relative overflow-hidden">
      ...
      <div className="absolute bottom-0 right-0 w-[600px] h-[600px] bg-accent/5 rounded-full blur-[120px] translate-x-1/3 translate-y-1/3 pointer-events-none"></div>
      ...
      <Card className="w-full max-w-md z-10 glass">
        <CardContent className="p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-text-muted mt-2">Log in to your VocalVitals account</p>
          </div>

          {errorText && <div className="text-danger text-sm mb-4 bg-danger/10 p-3 rounded">{errorText}</div>}
          ...
          <div className="mt-6 space-y-4">
            <Button variant="secondary" className="w-full" type="button" onClick={handleGoogleLogin}>
            ...
            <Button variant="ghost" className="w-full text-text-muted hover:text-white" type="button" onClick={handleGuestLogin}>
              Continue as Guest
            </Button>
          </div>
          ...
          <p className="mt-6 text-center text-xs text-text-muted/60 max-w-xs mx-auto">
            Voice data is processed locally and never stored without explicit consent.
          </p>
        </CardContent>
      </Card>
    </div>
